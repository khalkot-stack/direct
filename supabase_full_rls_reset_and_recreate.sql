...
-- First, disable RLS on all tables to ensure no policies interfere during drops
...
DROP POLICY IF EXISTS "Allow passengers to view driver profiles for their accepted rid" ON public.profiles;
...
--
-- RLS Policies for public.profiles (Minimal and safe to avoid recursion)
--
-- Only allow users to read, insert, and update their OWN profile.
-- Cross-profile reads (e.g., driver reading passenger's profile) will need to be handled
-- by ensuring the `rides` table RLS allows the join, and the `profiles` RLS is bypassed
-- for the joined data, or by fetching the data through a secure function.
-- For now, we prioritize eliminating recursion.
...
-- Allow authenticated users to delete their own messages" ON public.messages
