-- Disable RLS on all affected tables temporarily
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.rides DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.settings DISABLE ROW LEVEL SECURITY;

-- Drop all known RLS policies from profiles table
...
-- Drop all known RLS policies from rides table
...
-- Drop all known RLS policies from settings table
DROP POLICY IF EXISTS "Admins can view settings" ON public.settings;
DROP POLICY IF EXISTS "Admins can insert settings" ON public.settings;
DROP POLICY IF EXISTS "Admins can update settings" ON public.settings;

-- Drop the get_user_type function (now that dependent policies are gone)
DROP FUNCTION IF EXISTS public.get_user_type(uuid);

-- Re-create the get_user_type function
...
-- Re-create essential RLS policies for profiles (DO NOT use get_user_type here)
...
-- Re-create RLS policies for rides
...
-- Re-create RLS policies for settings
